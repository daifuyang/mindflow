...
这类方案写成文章时，需要注意不要把真正的公网 IP、真实域名、FRP token、完整 MAC、认证文件路径等信息直接公开。

公网 IP 和真实域名本身不等于密码，但它们会暴露服务入口，增加被扫描和撞库的概率。
...
|---|---|---|
| 架构链路 | 适合 | 分享方案价值 |
| 私有网段示例 | 基本可以 | `192.168.x.x` 不可公网访问 |
| 服务端口用途 | 谨慎 | 可以讲逻辑，但不必暴露真实公网入口 |
| 真实域名 | 不建议 | 会暴露入口 |
| 真实公网 IP | 不建议 | 会暴露入口 |
| FRP token | 绝对不能 | 等同隧道凭证 |
| Basic Auth 用户密码 | 绝对不能 | 等同登录凭证 |
| 完整 MAC 地址 | 不建议 | 没必要公开硬件标识 |
...
真正需要分享的是思路：**公网入口只负责传命令，OrangePi 在家里局域网内完成唤醒和转发。**
